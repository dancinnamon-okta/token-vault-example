 ...
 * @property {string} access_token - The external provider's access token
 * @property {string} scope - The scopes granted by the external provider
 * @property {number} expires_in - The number of seconds until the token expires
 * @property {string} issued_token_type - The type of token issued
 * @property {string} token_type - The token type (typically 'Bearer')
 */

/**
 * Performs an access token exchange with the Auth0 Token Vault.
 * 
 * This function exchanges an Auth0 access token for an external provider's access token.
 * The backend API authenticates itself using the Custom API Client credentials and
 * exchanges the subject token (Auth0 access token) for the requested external provider token.
 * 
 * @param {string} domain - The Auth0 domain (e.g., 'your-tenant.auth0.com')
 * @param {string} clientId - The Custom API Client ID
 * @param {string} clientSecret - The Custom API Client secret
 * @param {string} subjectToken - The Okta access token to exchange - this will be exchanged for an Auth0 one in this process
 * @param {string} connection - The connection name (e.g., 'google-oauth2')
 * @param {string} [loginHint] - Optional login hint for users with multiple accounts from the same connection
 * @returns {Promise<TokenExchangeResponse>} - The token exchange response containing the external access token
 * @throws {Error} - Throws an error if the token exchange fails
 * 
 * @example
 * const response = await exchangeAccessToken(
 *   'your-tenant.auth0.com',
 *   'YOUR_CUSTOM_API_CLIENT_ID',
 *   'YOUR_CUSTOM_API_CLIENT_SECRET',
 *   'AUTH0_ACCESS_TOKEN_FROM_SPA',
 *   'google-oauth2'
 * )
