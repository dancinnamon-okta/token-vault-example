...
async function completeConnectedAccountFlow(auth0Domain, proxyBaseUrl, authSession, userToken, connectCode) {

  // Complete the Connected Accounts request by calling Auth0's complete endpoint
  const completeUrl = `https://${auth0Domain}/me/v1/connected-accounts/complete`
  const redirectUri = `${proxyBaseUrl}/connected_account_callback`
  ...
  //If successful, auth0 passes back an ID parameter. We'll use that to determine success.
  if(response.data.id) {
    return
  }
  else {
    throw new Error("Unknown error occurred while completing the account linking flow.")
  }
